...
extern fp32 x_y_move_len;//, wz_move_len;
...
/**
  ...
  * @author         RM
  * @param[in]      vx_set前进 的速度 设定值将直接发送到can总线上
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
  ...
  //遥控器设置行为模式
  if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[ModeChannel]))
  {
    chassis_behaviour_mode = CHASSIS_FOLLOW_GIMBAL_YAW;
  }
  else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[ModeChannel]))
  {
