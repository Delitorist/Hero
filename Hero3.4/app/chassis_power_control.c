...
void chassis_power_control(chassis_move_t *chassis_power_control)
{
  ...
  {
    get_chassis_power_and_buffer(&chassis_power, &chassis_power_buffer);      //获取指针的函数被注释
    // power > 80w and buffer < 60j, because buffer < 60 means power has been more than 80w
    //功率超过80w 和缓冲能量小于j,因为缓冲能量小于60意味着功率超过80w
    if (chassis_power_buffer < WARNING_POWER_BUFF)
    {
      ...
      if (chassis_power_buffer > 5.0f)
      {
        ...
        power_scale = chassis_power_buffer / WARNING_POWER_BUFF;      //小于1
      }
      else
      {
        //only left 10% of WARNING_POWER_BUFF
        power_scale = 5.0f / WARNING_POWER_BUFF;                 //大于1
      }
      //scale down
      //缩小
    ...
    else
    {
      ...
      if (chassis_power > WARNING_POWER)
      {
        ...
        if (chassis_power < POWER_LIMIT)
        {
          //scale down
          //缩小
          power_scale = (POWER_LIMIT - chassis_power) / (POWER_LIMIT - WARNING_POWER);												//大于1
        }
        //power > 80w
        //功率大于80w
        else
